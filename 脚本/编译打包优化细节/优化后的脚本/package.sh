...
# function target_file_copy_bin()
# {
#     echo "bin"
#     cd /root/software_resources/openGauss-server/dest/
#     for file in `ls $1`
#     do
...
# }
# function target_file_copy_share()
# {
#     echo "share"
#     cd /root/software_resources/openGauss-server/dest/
#     for file in `ls $1`
...
function target_file_copy()
{
    ...
    # chmod 777 /root/software_resources/openGauss-server/dest/bin
    # cd ${BUILD_DIR}
    # rm -rf ./include
    # rm -rf ./lib
    # cd /root/
    ...
    for file in ./*
    do
        copy_files_list $file ${BUILD_DIR}/temp/bin
    done
    #复制./etc
    cd ${BUILD_DIR}/etc
    for file in ./*
    do
        copy_files_list $file ${BUILD_DIR}/temp/etc
    done
    #复制./share
    cd ${BUILD_DIR}/share
    for file in ./*
    ...
    echo $file
    echo "哈哈"
        copy_files_list $file ${BUILD_DIR}/temp/share
    done
    ...
    #把准备好的临时文件直接拷贝到这里，加快打包速度 for file in /root/software_resources/openGauss-server/dest/share/*
    echo "把其他位置的目录文件复制到这边"
    cp -r /root/software_resources/openGauss-server/temp_files/lib   /root/software_resources/openGauss-server/dest/temp
    cp -r /root/software_resources/openGauss-server/temp_files/include   /root/software_resources/openGauss-server/dest/temp
    # rm -rf /root/software_resources/temp_files/share
    # cp -rf /root/software_resources/openGauss-server/dest/share   /root/software_resources/openGauss-server/dest/temp
    echo "371"
    # exit 0
    read_script_file
    cd $SCRIPT_DIR
    ...
    echo "将/root/software_resources/openGauss-server/dest/temp/bin/cluster_guc.conf文件复制过去"
    # cp -rf /root/software_resources/openGauss-server/dest/bin/cluster_guc.conf /root/software_resources/openGauss-server/dest/temp/bin/
    echo "430"
...
function make_package_srv()
{
    ...
    chmod -R 777 ${BUILD_DIR}/temp/etc
    echo "523\n"
    chmod -R 777 /root/software_resources/openGauss-server/dest/bin
    chmod -R  777 /root/software_resources/openGauss-server/dest/etc
    chmod -R 777 /root/software_resources/openGauss-server/dest/include
    chmod -R 777 /root/software_resources/openGauss-server/dest/lib
    chmod -R 777 /root/software_resources/openGauss-server/dest/script
    chmod -R 777 /root/software_resources/openGauss-server/dest/share
    chmod -R 777 /root/software_resources/openGauss-server/dest/temp
    # target_file_copy_bin "$copydest_bin" ${BUILD_DIR}/temp
    # target_file_copy_etc "$copydest_etc" ${BUILD_DIR}/temp
    # target_file_copy_share "$copydest_share" ${BUILD_DIR}/temp
