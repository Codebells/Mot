...
bool OccTransactionManager::QuickVersionCheck_1(const Access* access)
{
    // We always validate on commited rows!
    const Row* row = access->GetRowFromHeader();
    return row->m_rowHeader.ValidateWrite_1(access,access->m_tid);
}
...
bool OccTransactionManager::ValidateReadSet_1(TxnManager* txMan)
{
    TxnOrderedSet_t& orderedSet = txMan->m_accessMgr->GetOrderedRowSet();
    for (const auto& raPair : orderedSet) {
        const Access* ac = raPair.second;
        ...
        if (!!ac->GetRowFromHeader()->m_rowHeader.ValidateRead_1(ac,ac->m_tid))
        {
            return false;
        }
...
bool OccTransactionManager::ValidateWriteSet_1(TxnManager* txMan)
{
    TxnOrderedSet_t& orderedSet = txMan->m_accessMgr->GetOrderedRowSet();
    for (const auto& raPair : orderedSet) {
        const Access* ac = raPair.second;
        ...
        if (!ac->GetRowFromHeader()->m_rowHeader.ValidateWrite_1(ac,ac->m_tid))
        {
            return false;
        }
    }
...
bool OccTransactionManager::ValidateEpoch_1(TxnManager * txMan){
    MOT_LOG_INFO("进行Epoch验证，如果事务的开始时间，小于行的当前时间戳，则说明该epoch有其他事务更改了这个行，长事务被abort");
        TxnOrderedSet_t &orderedSet = txMan->m_accessMgr->GetOrderedRowSet();
    bool result = true;
    for (const auto &raPair : orderedSet)
    {
        ...
        //需要重新写一遍！！！！！！！！！！
        if (!ac->GetRowFromHeader()->m_rowHeader.ValidateEpoch_1(ac,txMan->GetStartEpoch()))
        {
            return false;
        }
...
//只读事务
bool OccTransactionManager::ValidateReadInMerge_1(TxnManager * txMan){
   MOT_LOG_INFO("验证开始在merge期间执行的事务");
    TxnOrderedSet_t &orderedSet = txMan->m_accessMgr->GetOrderedRowSet();
    bool result = true;
    for (const auto &raPair : orderedSet)
    {
        ...
        if (ac->m_type == RD)
        {
            if (!ac->GetRowFromHeader()->m_rowHeader.ValidateRead_1(ac,ac->m_cts))
            {
                return false;
            }
        }
...
bool OccTransactionManager::ValidateAndSetWriteSet_1(TxnManager *txMan)
{
    MOT_LOG_INFO("验证并更新CSN ");
    TxnOrderedSet_t &orderedSet = txMan->m_accessMgr->GetOrderedRowSet();
    bool result = true;
    for (const auto &raPair : orderedSet)
    {
        ...
        if (!ac->GetRowFromHeader()->m_rowHeader.ValidateAndSetWrite_1(ac,txMan->GetCommitSequenceNumber(), txMan->GetStartEpoch()))
        {
            return false;
        }

        MOT_LOG_INFO("==ValidateAndSetWriteSet_1:验证并更新CSN done.");
...
bool OccTransactionManager::aria_ValidateAndSetWriteSet(TxnManager *txMan)
{
    MOT_LOG_INFO("验证并更新CSN ");
    TxnOrderedSet_t &orderedSet = txMan->m_accessMgr->GetOrderedRowSet();
    bool result = true;
    for (const auto &raPair : orderedSet)
    {
        ...
        if (!ac->GetRowFromHeader()->m_rowHeader.aria_ValidateAndSetWrite(txMan->GetCommitSequenceNumber(), txMan->GetStartEpoch()))
        {
            result = false;
        }
        if(ac->m_type == INS){
...
bool OccTransactionManager::ValidateWriteSetII_1(TxnManager *txMan)
{
    TxnOrderedSet_t &orderedSet = txMan->m_accessMgr->GetOrderedRowSet();
    for (const auto &raPair : orderedSet)
    {
        ...
        //     return false;
        // }
        if (!ac->GetRowFromHeader()->m_rowHeader.ValidateWrite_1(ac,txMan->GetCommitSequenceNumber()))
        {
            return false;
        }
...
//判断是否有WAW,RAW,WAR
bool OccTransactionManager::aria_ValidateWriteSetII(TxnManager *txMan)
{
    bool RAW = false;
    bool WAR = false;
    ...
    for (const auto &raPair : orderedSet)
    {
        ...
        if (ac->m_type == RD )
        {
            if (!ac->GetRowFromHeader()->m_rowHeader.check_RAW(txMan->GetCommitSequenceNumber(), txMan->GetStartEpoch()))
            {
                RAW = true;
                //同时存在RAW和WAR依赖
                if (WAR)
        ...
        {
            //WAW
             if (!ac->GetRowFromHeader()->m_rowHeader.check_WAW(txMan->GetCommitSequenceNumber(), txMan->GetStartEpoch()))
            {
                return false;
            }
            ...
            if (ac->m_type == WR)
            {
                if (!ac->GetRowFromHeader()->m_rowHeader.check_WAR(txMan->GetCommitSequenceNumber(), txMan->GetStartEpoch()))
                {
                    WAR = true;
                    if (RAW)
                    {
...
RC OccTransactionManager::ExecutionPhase_1(TxnManager *txMan)
{
    TxnAccess *tx = txMan->m_accessMgr.Get();
    RC rc = RC_OK;
    const uint32_t rowCount = tx->m_rowCnt;
    ...
    bool result = ValidateAndSetWriteSet_1(txMan);
    if (!result)
    {
        MOT_LOG_INFO("尝试更新CTS失败");
        return RC::RC_ABORT;
    }
    if(!ValidateEpoch_1(txMan)){
        MOT_LOG_INFO("EPoch验证未通过，长事务被abort");
        return RC::RC_ABORT;
    }
    return RC::RC_OK;
...
RC OccTransactionManager::aria_ExecutionPhase(TxnManager *txMan)
{
    TxnAccess *tx = txMan->m_accessMgr.Get();
    RC rc = RC_OK;
    const uint32_t rowCount = tx->m_rowCnt;
    ...
    //reserve reads set
    aria_reserveRead(txMan);

    //reserve writes set
    bool result = aria_ValidateAndSetWriteSet(txMan);
    if (!result)
    {
        MOT_LOG_INFO("尝试更新CTS失败");
        return RC::RC_ABORT;
...
void OccTransactionManager::aria_reserveRead(TxnManager *txMan)
{
    TxnAccess *tx = txMan->m_accessMgr.Get();
    TxnOrderedSet_t &orderedSet = tx->GetOrderedRowSet();
     for (const auto &raPair : orderedSet)
    {
        ...
        if (ac->m_type == RD)
        {
            ac->GetRowFromHeader()->m_rowHeader.reserveRead(txMan->GetCommitSequenceNumber(), txMan->GetStartEpoch());    
        }
    }
}
...
RC OccTransactionManager::PreCommitPhase_1(TxnManager *txMan)
{
    MOT_LOG_INFO("PreCommit Phase");
    bool result = ValidateWriteSetII_1(txMan);
    if (result) //precommit success 
    {
...
RC OccTransactionManager::aria_PreCommitPhase(TxnManager *txMan)
{
    MOT_LOG_INFO("PreCommit Phase");
    bool result = aria_ValidateWriteSetII(txMan);
    if (result) //precommit success 
    {
...
RC OccTransactionManager::CommitPhase_1(TxnManager *txMan)
{
    MOT_LOG_INFO("Commit Phase");
    bool result = ValidateWriteSetII_1(txMan);
    if (result) //commit writeChange
    {
        return txMan->CommitInternalII_1();
    }
    MOT_LOG_INFO("最终提交失败");
    return RC::RC_ABORT;
}
...
RC OccTransactionManager::aria_CommitPhase(TxnManager *txMan)
{
    MOT_LOG_INFO("Commit Phase");
    bool result = aria_ValidateWriteSetII(txMan);
    if (result) //commit writeChange
...
RC OccTransactionManager::LockRows_1(TxnManager* txMan, uint32_t& numRowsLock)
{
    RC rc = RC_OK;
    TxnOrderedSet_t& orderedSet = txMan->m_accessMgr->GetOrderedRowSet();
    numRowsLock = 0;
...
bool OccTransactionManager::WriteChanges_1(TxnManager* txMan)
{
    if (m_writeSetSize == 0 && m_insertSetSize == 0) {
        return true;
    }
    ...
    for (const auto& raPair : orderedSet) {
        const Access* access = raPair.second;
        access->GetRowFromHeader()->m_rowHeader.WriteChangesToRow_1(access, txMan->GetCommitSequenceNumber());
    }
