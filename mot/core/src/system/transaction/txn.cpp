...
RC TxnManager::StartTransaction(uint64_t transactionId, int isolationLevel)
{
    ...
    MOT_LOG_INFO("开始事务!");
    SetStartTS(startts);
    SetStartEpoch(MOTAdaptor::GetEpoch());
    SetStartInMerge(MOTAdaptor::isInMerge());
    MOT_LOG_INFO(("Start epoch:" + std::to_string(GetStartEpoch())).c_str());
    MOT_LOG_INFO(("if Start in Merge?:" + std::to_string(GetStartInMerge())).c_str());
    GcSessionStart();
    return RC_OK;
}
...
RC TxnManager::ValidateOcc(uint64_t transcationId)
{
...
bool TxnManager::localMergeValidate(uint64_t csn)
{
    ...
    RC TxnManager::CommitInternalII()
    {
        //MOTAdaptor::InsertRowToSet(this);
        // Record the start write phase for this transaction
        if (GetGlobalConfiguration().m_enableCheckpoint)
        {
            GetCheckpointManager()->BeginTransaction(this);
...
{
    MOT_LOG_INFO("start commit txn");
    if(GetStartInMerge()){
        MOT_LOG_INFO("在merge期间开始的事务，阻塞直到merge结束");
        while(MOTAdaptor::isInMerge());
    }
    if (transcationId != INVALID_TRANSACTIOIN_ID)
            m_transactionId = transcationId;
    ...
    if (this->m_accessMgr->m_rowCnt > 0 && !result)
    {
        ...
        SetCommitTS(GetCommitSequenceNumber());
        while(MOTAdaptor::isLocalExeced());
        MOTAdaptor::IncPreCounter();
        MOTAdaptor::IncExeCounter();
        MOTAdaptor::IncComCounter();
        
        if(!m_occManager.ValidateReadInMerge(this)){
            return RC_ABORT;
        }
        uint64_t currentEpoch = MOTAdaptor::GetEpoch();
        MOT_LOG_INFO("currentEpoch = %lu", currentEpoch);
        MOT_LOG_INFO("execute commit txn");
        RC rc = m_occManager.ExecutionPhase(this);
        if (rc != RC_OK)
        {
            MOTAdaptor::DecExeCounter();
            MOTAdaptor::DecPreCounter();
            MOTAdaptor::DecComCounter();
            return rc;
        }
        MOTAdaptor::DecExeCounter();
        MOT_LOG_INFO("执行阶段完成")
        while(!MOTAdaptor::isLocalExeced() || MOTAdaptor::GetExeCounter() != 0); 
        MOT_LOG_INFO("开始预提交阶段");
        rc = m_occManager.PreCommitPhase(this);
        if (rc != RC_OK)
        {
            MOTAdaptor::DecComCounter();
            MOTAdaptor::DecPreCounter();
            ...
            MOTAdaptor::InsertRowToSet(this);//1
        MOTAdaptor::DecPreCounter();//才有机会变成０
        MOT_LOG_INFO("预提交阶段结束，阻塞等待发送完成");
        while(!MOTAdaptor::isRemoteExeced()|| !MOTAdaptor::isMergeSent());
        MOT_LOG_INFO("开始最后提交阶段");
        rc = m_occManager.CommitPhase(this);
        if (rc == RC_OK)
        {
            MOT::DbSessionStatisticsProvider::GetInstance().AddCommitTxn();
        }
        // while (!MOTAdaptor::ismergeSent());
        MOTAdaptor::DecComCounter();
        MOT_LOG_INFO("这个事务提交完成");
        return rc;
    }
    else
    {
