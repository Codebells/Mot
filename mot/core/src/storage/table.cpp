...
RC Table::InsertRow(Row* row, TxnManager* txn)
{
    ...
    //add primaryKey to this row 
    MOT_LOG_INFO(("row key length:"+ std::to_string(key->GetKeyLength())).c_str());
    // row->SetPrimaryKey(key->GetKeyStr());
    // MOT_LOG_INFO(("table add GetKeyStr:"+ row->GetPrimaryKey()).c_str());
    row->SetPKey(key->GetKeyStr());
    MOT_LOG_INFO(("table add key:"+ row->GetPKey()).c_str());
    // add secondary indexes
    for (uint16_t i = 1; i < numIndexes; i++) {
        ix = GetSecondaryIndex(i);
        key = txn->GetTxnKey(ix);
...
//创建时添加start时间戳
Row* Table::CreateNewRow(uint64_t timestamp)
{
    Row* row = m_rowPool->Alloc<Row>(this);
    if (row == nullptr) {
        MOT_REPORT_ERROR(MOT_ERROR_OOM, "Create Row", "Failed to create new row in table %s", m_longTableName.c_str());
    }
    row->SetStartTS(timestamp);
    return row;
}
