...
unordered_set<MOT::TxnManager *> MOTAdaptor::local_changeset;
std::unordered_map<int, MOT::Row *> MOTAdaptor::remote_changeset;
std::unordered_map<void *, uint64_t> MOTAdaptor::insertSet;
pthread_t MOTAdaptor::pMerge;
pthread_t MOTAdaptor::pMergeProcess;
///
uint64_t MOTAdaptor::epoch = 0;
std::atomic<uint64_t> MOTAdaptor::exeCounter(0);
std::atomic<uint64_t> MOTAdaptor::preCounter(0);
std::atomic<uint64_t> MOTAdaptor::comCounter(0);
pthread_t MOTAdaptor::pTimer;
bool MOTAdaptor::timerStop = false;
bool MOTAdaptor::localExeced = false;
bool MOTAdaptor::remoteExeced = false;
bool MOTAdaptor::remoteComitted = false;
bool MOTAdaptor::mergeSent = false;
bool MOTAdaptor::inMerge = false;
//size_t MOTAdaptor::sleepTime = 200000;
size_t MOTAdaptor::sleepTime = 1000000;
...
///
void *MOTAdaptor::TimerThread(void *params)
{
    while (!timerStop)
    {
        usleep(sleepTime);
        MOTAdaptor::setInMerge(true);
        MOTAdaptor::callback();
        MOTAdaptor::setInMerge(false);
    }
    MOT_LOG_INFO("stop = %b", timerStop);
}

void MOTAdaptor::callback()
{
    MOT_LOG_INFO("开始merge");
    MOTAdaptor::setLocalExeced(true);

    // send merge
    //MOTAdaptor::RequestMergeThread();
    MOTAdaptor::RequestMergeThread_1();

    while (MOTAdaptor::GetComCounter() != 0 || !MOTAdaptor::isRemoteCommited());
    MOTAdaptor::IncEpoch();
    MOT_LOG_INFO("epoch = %lu", GetEpoch());
    MOTAdaptor::setLocalExeced(false);
    MOTAdaptor::setRemoteExeced(false);
    MOTAdaptor::setRemoteCommited(false);
    MOTAdaptor::setMergeSent(false);
}
///
...
void MOTAdaptor::Init()
{
    ...
    if (MOTAdaptor::startMergeThread() != 0)
    {
        elog(FATAL, "Failed to initialize process merge thread.");
    }
    //启动计时器线程
    if (MOTAdaptor::startTimerThread() != 0)
    {
        elog(FATAL, "Failed to initialize timer thread.");
    }
...
/********************************************/
static void get_addres(vector<string> &res)
{
    MOT_LOG_INFO("获取IP.....");
    string tmp;

    fstream fp;
    fp.open("/tmp/ip_addres.txt");
...
static string get_local()
{
    string res;
    fstream fp;
    fp.open("/tmp/local_ip.txt");
    getline(fp, res);
    fp.close();
    return res;
}
...
void MOTAdaptor::RequestMergeThread()
{
    /**************************************/
    vector<string> addres;
    get_addres(addres); 
    int addr_size = addres.size();
    /**************************************/
    // MOT_LOG_INFO((string("地址列表大小:")+std::to_string(addr_size)).c_str());
    // MOT_LOG_INFO("以下是所有的地址:");
    for (int i=0; i<addr_size; i++)
    {
        MOT_LOG_INFO(addres[i].c_str());
    }
    ...
    // //request_pusher.connect("tcp://172.16.181.130:5557");
    //MOT_LOG_INFO("while之前");
    while (MOTAdaptor::GetPreCounter() != 0);

    MOT_LOG_INFO("set大小 = %lu", local_changeset.size());

    if (local_changeset.size() > 0)
    {
        MOT_LOG_INFO("发送Merge");
        MergeRequest mergereq;
        for (auto it = local_changeset.begin(); it != local_changeset.end(); it++)
        {
            MOT::TxnManager *txnMan = (*it);
            MOT::TxnOrderedSet_t &orderedSet = txnMan->m_accessMgr->GetOrderedRowSet();
            merge::MergeRequest_Transaction *Txn = mergereq.add_txn();

            for (const auto &raPair : orderedSet)
            {

                const MOT::Access *access = raPair.second;
                merge::MergeRequest_Transaction_Row *row = Txn->add_row();
                if (access->m_type == MOT::INS)
                {
                    MOT::Row *insertRow = access->m_localInsertRow;
                    std::string pKey = insertRow->GetPKey();
                    uint8_t m_keyBuf[pKey.size() / 2];
                    uint32_t pos = 0;
                    for (size_t i = 0; i < pKey.size(); i++)
                    {
                        m_keyBuf[pos++] = (((uint8_t)(pKey[i] - '0') & 0x0F) << 4) | (((uint8_t)(pKey[++i] - '0') & 0x0F));
                    }
                    row->set_tablename(insertRow->GetTable()->GetLongTableName());
                    row->set_key((void *)m_keyBuf, insertRow->GetPKey().size() / 2);
                    row->set_data(insertRow->GetData(), insertRow->GetTable()->GetTupleSize());

                    std::string res = MOT::HexStr((uint8_t *)m_keyBuf, pKey.size() / 2);
                    MOT_LOG_INFO(("send_key:" + res).c_str());
                    MOT_LOG_INFO(("tableName" + insertRow->GetTable()->GetLongTableName()).c_str());
                }
                else if (access->m_type == MOT::WR)
                {
                    MOT::Row *updateRow = access->m_localRow;
                    std::string pKey = updateRow->GetPKey();
                    uint8_t m_keyBuf[pKey.size() / 2];
                    uint32_t pos = 0;
                    for (size_t i = 0; i < pKey.size(); i++)
                    {
                    ...
                    row->set_tablename(updateRow->GetTable()->GetLongTableName());
                    row->set_key((void *)m_keyBuf, pKey.size() / 2);
                    row->set_data(updateRow->GetData(), updateRow->GetTable()->GetTupleSize());

                    std::string res = MOT::HexStr((uint8_t *)m_keyBuf, pKey.size() / 2);
                    MOT_LOG_INFO(("send_key:" + res).c_str());
                    MOT_LOG_INFO(("tableName" + updateRow->GetTable()->GetLongTableName()).c_str());
                }
                else if (access->m_type == MOT::DEL)
                {
                    MOT::Row *DeleteRow = access->m_localRow;
                    std::string pKey = DeleteRow->GetPKey();
                    uint8_t m_keyBuf[pKey.size() / 2];
                    uint32_t pos = 0;
                    for (size_t i = 0; i < pKey.size(); i++)
                    {
                    ...
                    row->set_tablename(DeleteRow->GetTable()->GetLongTableName());
                    row->set_key((void *)m_keyBuf, pKey.size() / 2);
                    // row->set_data(updateRow->GetData(), updateRow->GetTable()->GetTupleSize());

                    std::string res = MOT::HexStr((uint8_t *)m_keyBuf, pKey.size() / 2);
                    MOT_LOG_INFO(("send_key:" + res).c_str());
                    MOT_LOG_INFO(("tableName" + DeleteRow->GetTable()->GetLongTableName()).c_str());
                }
            }
            Txn->set_startts(txnMan->GetStartTS());
            Txn->set_committs(txnMan->GetCommitTS());
            Txn->set_txnid(txnMan->GetInternalTransactionId());
        }

        /***********************************************/
        //mergereq.set_request_address("172.16.181.128");
        mergereq.set_request_address(get_local());
        /***********************************************/

        // MOT_LOG_INFO("准备发送....");
        MOTAdaptor::ClearChangeSet();
        std::string serialized_req;
        
        mergereq.SerializeToString(&serialized_req);
        zmq::message_t msg(serialized_req.size());
        memcpy(msg.data(), serialized_req.c_str(), serialized_req.size());
        ...
        for (int ip_cnt = 0; ip_cnt < addr_size; ip_cnt ++ )
        {
            zmq::socket_t request_pusher(context, ZMQ_PUSH);
            // tcp://172.16.181.130:5557
            request_pusher.connect("tcp://"+addres[ip_cnt]+":5557");
    ...
    MOT_LOG_INFO("发送完成....");
    MOTAdaptor::setMergeSent(true);
}

void MOTAdaptor::ProcessMergeThread()
{
    MOT_LOG_INFO("进入ProcessMergeThread函数.....");
    zmq::context_t context(1);
    int remoteServerNum = 1;
    //zmq_ctx_set(&context, ZMQ_IO_THREADS, 3);
    // 负责pull　resquest
    zmq::socket_t request_puller(context, ZMQ_PULL);
    request_puller.bind("tcp://*:5557");
    std::string remoteMsgs[remoteServerNum];
    for (;;)
    {
        for (int i = 0; i < remoteServerNum; i++)
        {
            zmq::message_t message;
            request_puller.recv(&message);
            std::string res = string(static_cast<const char *>(message.data()), message.size());
            remoteMsgs[i] = res;
            MOT_LOG_INFO((string("接收到消息:")+res).c_str());
        }
        while (MOTAdaptor::GetPreCounter() != 0);
        int index = 0;
        for (int i = 0; i < remoteServerNum; i++)
        {
            ...
            if (remoteMsgs[i] == " ")
                continue;
            MOT_LOG_INFO("execution stage");
            mergereq.ParseFromString(remoteMsgs[i]);

            //遍历transaction，为rowHeader打上csn
            for (int i = 0; i < mergereq.txn_size(); i++)
            {
                merge::MergeRequest_Transaction Txn = mergereq.txn(i);
                for (int j = 0; j < Txn.row_size(); j++)
                {
                    MOT::Table *table = m_engine->GetTableManager()->GetTable(Txn.row(j).tablename());
                    int KeyLength = Txn.row(j).key().length();
                    MOT::Key *key = new MOT::Key(KeyLength);
                    key->CpKey((uint8_t *)Txn.row(j).key().c_str(), KeyLength);
                    MOT_LOG_INFO(("recv_key:" + key->GetKeyStr()).c_str());
                    MOT_LOG_INFO(("recv_tableName" + Txn.row(j).tablename()).c_str());
                    MOT::Row *localRow = nullptr;
                    uint64_t csn = Txn.committs();
                    table->FindRow(key, localRow, 0);
                    remote_changeset.insert(std::pair<int, MOT::Row *>(index++, localRow));
                    if (localRow != nullptr)
                    {
                        MOT_LOG_INFO(std::to_string(csn).c_str());
                        MOT_LOG_INFO(std::to_string(localRow->GetCommitSequenceNumber()).c_str());
                        if (csn > localRow->GetCommitSequenceNumber())
                            localRow->SetCommitSequenceNumber(csn);
                        std::string res = key->GetKeyStr();
                        MOT_LOG_INFO(res.c_str());
                    }
                    // else{
                    //         MOT_LOG_INFO("需要插入新row");
                    //         MOT::Row* row = table->CreateNewRow();
                    //         row->CopyData((uint8_t*)Txn.row(j).data().c_str(),table->GetTupleSize());

                    //         MOT::SessionContext* session_ctx = m_engine->GetSessionManager()->CreateSessionContext(IS_PGXC_COORDINATOR, 0, nullptr);
                    //         MOT::TxnManager* tmpTxn = session_ctx->GetTxnManager();
                    //         // MOT::RC res = table->InsertRow(row, tmpTxn);
                    //         // tmpTxn->Commit();
                    // }
                }
            }
        ...
        for (int i = 0; i < remoteServerNum; i++)
        {
            if (remoteMsgs[i] == " ")
                continue;
            MOT_LOG_INFO("commit stage");
            MergeRequest mergereq;
            mergereq.ParseFromString(remoteMsgs[i]);
            //再次遍历，检查csn是否被更改，被更改则终止本轮遍历,相当于abort这个tansaction,　对可以提交的．其相应的bool值为true
            for (int i = 0; i < mergereq.txn_size(); i++)
            {
                merge::MergeRequest_Transaction Txn = mergereq.txn(i);
                int commitNum = Txn.row_size();
                for (int j = 0; j < Txn.row_size(); j++)
                {
                    MOT::Row *localRow = remote_changeset.find(index++)->second;
                    uint64_t csn = Txn.committs();
                    if (localRow != nullptr)
                    {
                        MOT_LOG_INFO(std::to_string(csn).c_str());
                        MOT_LOG_INFO(std::to_string(localRow->GetCommitSequenceNumber()).c_str());
                        if (localRow->GetCommitSequenceNumber() != csn)
                        {
                            commitNum--;
                            break;
                        }
                ...
                //将可以提交的remote txn写到本地数据库
                if (commitNum == Txn.row_size())
                {
                    index -= commitNum;
                    for (int j = 0; j < Txn.row_size(); j++)
                    {
                        ...
                        if (localRow != nullptr)
                        {
                            if (Txn.row(j).data().length() == 0)
                                localRow->GetPrimarySentinel()->SetDirty();
                            else
                                localRow->CopyData((uint8_t *)Txn.row(j).data().c_str(), table->GetTupleSize());
                        }
                        else
                        {
                            //插入新row
                ...
                {
                    index = index - commitNum + Txn.row_size();
                }
            }
        }
        MOTAdaptor::setRemoteCommited(true);
        remote_changeset.clear();
    }
}
...
void MOTAdaptor::RequestMergeThread_1()
{
    /**************************************/
    vector<string> addres;
    get_addres(addres); 
    ...
    MOT_LOG_INFO("1.set大小 = %lu", local_changeset.size());

    if (local_changeset.size() > 0)
    {
        MOT_LOG_INFO("1.发送Merge");
        //MergeRequest mergereq;
        merge::MergeRequestNew mergereq;
        for (auto it = local_changeset.begin(); it != local_changeset.end(); it++)
        {
            MOT::TxnManager *txnMan = (*it);
            MOT::TxnOrderedSet_t &orderedSet = txnMan->m_accessMgr->GetOrderedRowSet();
            merge::MergeRequestNew_Transaction *Txn = mergereq.add_txn();

            for (const auto &raPair : orderedSet)
            {

                const MOT::Access *access = raPair.second;
                merge::MergeRequestNew_Transaction_Row *row = Txn->add_row();
                if (access->m_type == MOT::INS)
                {
                    MOT::Row *insertRow = access->m_localInsertRow;
                    std::string pKey = insertRow->GetPKey();

                    MOT_LOG_INFO((string("1.INS中的pKey:")+pKey).c_str());
                    ...
                    /*****************************/
                    //如果为INS类型，那么则发送所有的数据
                    uint64_t fieldCnt = insertRow->GetTable()->GetFieldCount();
                    MOT_LOG_INFO((string("1.field_cnt:")+std::to_string(fieldCnt)).c_str());
                    for (int field_i = 0; field_i < fieldCnt; field_i ++)
                    {
                        merge::MergeRequestNew_Transaction_Row_Column* col = row->add_col();
                        col->set_id(field_i);
                        col->set_value(insertRow->GetValue(field_i),insertRow->GetTable()->GetFieldSize(field_i));   
                        //MOT_LOG_INFO((string("1.Col = ")+std::to_string(field_i)+string(" ,ColValue = ")+col->value()).c_str());
                        MOT_LOG_INFO((string("1.Col = ")+std::to_string(field_i)+ string(",col_size = ") + 
                            std::to_string(col->value().length()) + string(",table_col_size = ") + std::to_string(insertRow->GetTable()->GetFieldSize(field_i)) 
                            ).c_str());
                    ...
                    std::string res = MOT::HexStr((uint8_t *)m_keyBuf, pKey.size() / 2);
                    MOT_LOG_INFO(("1.send_key:" + res).c_str());
                    MOT_LOG_INFO(("1.tableName" + insertRow->GetTable()->GetLongTableName()).c_str());
                }
                else if (access->m_type == MOT::WR)
                {
                    MOT::Row *updateRow = access->m_localRow;
                    std::string pKey = updateRow->GetPKey();
                    MOT_LOG_INFO((string("1.WR中的pKey:")+pKey).c_str());
                    uint8_t m_keyBuf[pKey.size() / 2];
                    uint32_t pos = 0;
                    for (size_t i = 0; i < pKey.size(); i++)
                    {
                    ...
                    /*******************************/
                    //原来是直接将这一行复制
                    uint64_t fieldCnt = updateRow->GetTable()->GetFieldCount();
                    ...
                    for (uint16_t field_i = 0; field_i < fieldCnt-1; field_i ++)
                    {
                        ...
                        {
                            int real_filed = field_i+1;
                            // merge::MergeRequestNew_Transaction_Row_Column* col = row->add_col();
                            // col->set_id(field_i);
                            // col->set_value(updateRow->GetValue(field_i),updateRow->GetTable()->GetFieldSize(field_i));
                            // MOT_LOG_INFO((string("1.Col = ")+std::to_string(field_i)+ string(",col_size = ") + 
                            // std::to_string(col->value().length()) + string(",table_col_size = ") + std::to_string(updateRow->GetTable()->GetFieldSize(field_i))
                            // ).c_str());

                            merge::MergeRequestNew_Transaction_Row_Column* col = row->add_col();
                            col->set_id(real_filed);
                            col->set_value(updateRow->GetValue(real_filed),updateRow->GetTable()->GetFieldSize(real_filed));
                            MOT_LOG_INFO((string("1.Col = ")+std::to_string(real_filed)+ string(",col_size = ") + 
                            std::to_string(col->value().length()) + string(",table_col_size = ") + std::to_string(updateRow->GetTable()->GetFieldSize(real_filed))
                            ).c_str());
...
void MOTAdaptor::RequestMergeThread_2(){
   
    /***********************/
    MOT_LOG_INFO("1.进入RequestMergeThread_1.....");
    vector<string> addres;
    get_addres(addres); 
    int addr_size = addres.size();
    MOT_LOG_INFO((string("1.地址列表大小:")+std::to_string(addr_size)).c_str());
    MOT_LOG_INFO("1.以下是所有的地址:");
    for (int i=0; i<addr_size; i++)
    {
        MOT_LOG_INFO(addres[i].c_str());
    ...
    //MOT_LOG_INFO("1.while之前");
    while(MOTAdaptor::GetPreCounter() != 0);
    //MOT_LOG_INFO("1.while之后");
    ...
    if (local_changeset.size() > 0) {
        ...
        for (auto it = local_changeset.begin(); it != local_changeset.end(); it++) {
            ...
            for (const auto& raPair : orderedSet) {
                ...
                merge::MergeRequestNew_Transaction_Row* row = Txn->add_row();
                if(access->m_type==MOT::INS){ 
                    // ereport(LOG,(errmsg("InsertRequest执行")));
                    MOT_LOG_INFO("1.Insert Request执行");
                    MOT:: Row* insertRow = access->m_localInsertRow;
                    std::string pKey = insertRow->GetPKey();
                    uint8_t m_keyBuf[pKey.size()/2];
                    uint32_t pos = 0;
                    ...
                    //row->set_tableexid(insertRow->GetTable()->GetTableExId());
                    row->set_tablename(insertRow->GetTable()->GetLongTableName());
                    row->set_key((void *)m_keyBuf,insertRow->GetPKey().size()/2);
                    ...
                    for (int field_i = 0; field_i < fieldCnt; field_i ++)
                    {
                        ...
                        MOT_LOG_INFO((string("1.Col = ")+std::to_string(field_i)+string(" , ColId = ")+string(" ,ColValue = ")+col->value()).c_str());
                    ...
                    std::string res = MOT::HexStr((uint8_t*)m_keyBuf, pKey.size()/2);
                    MOT_LOG_INFO(("1.send_key:"+ res).c_str());
                    MOT_LOG_INFO((string("1.操作类型")+std::to_string(access->m_type)).c_str());
                    
                }
                else if(access->m_type==MOT::WR){
                    
                    MOT_LOG_INFO("1.WR Request执行");
                    MOT::Row* updateRow = access->m_localRow;
                    std::string pKey = updateRow->GetPKey();
                    MOT_LOG_INFO((string("1.WR中的pKey:")+pKey).c_str());
                    uint8_t m_keyBuf[pKey.size()/2];
                    ...
                    //row->set_tableexid(updateRow->GetTable()->GetTableExId());
                    row->set_tablename(updateRow->GetTable()->GetLongTableName());
                    row->set_key((void *)m_keyBuf, pKey.size()/2);
                    MOT_LOG_INFO((string("1.key_size = ")+std::to_string(pKey.size()/2)).c_str());
                    MOT_LOG_INFO((string("1.key = ")+row->key()+string(",key_size = ")+std::to_string(pKey.size()/2)).c_str());
                    ...
                    for (uint16_t field_i = 0; field_i < fieldCnt; field_i ++)
                    {
                        if (bmp.GetBit(field_i))
                        {
                            ...
                            col->set_id(field_i);
                            col->set_value(updateRow->GetValue(field_i),updateRow->GetTable()->GetFieldSize(field_i));
                            MOT_LOG_INFO((string("1.第 ")+std::to_string(field_i)+string(" 列modified，ColId = ")+std::to_string(col->id())+string(", ColValue = ")+col->value()).c_str());
                    ...
                    // ereport(LOG,(errmsg("DeleteRequest执行")));
                    MOT_LOG_INFO("1.DeleteRequest执行");
                    MOT::Row* DeleteRow = access->m_localRow;
                    std::string pKey = DeleteRow->GetPKey();
                    uint8_t m_keyBuf[pKey.size()/2];
                    uint32_t pos = 0;
                    ...
                    //row->set_tableexid(DeleteRow->GetTable()->GetTableExId());
                    row->set_tablename(DeleteRow->GetTable()->GetLongTableName());
                    row->set_key((void *)m_keyBuf, pKey.size()/2);
                    // row->set_data(updateRow->GetData(), updateRow->GetTable()->GetTupleSize());
...
void MOTAdaptor::ProcessMergeThread_1()
{
    MOT_LOG_INFO("2.进入ProcessMergeThread函数.....");
    zmq::context_t context(1);
    int remoteServerNum = 1;
    //zmq_ctx_set(&context, ZMQ_IO_THREADS, 3);
    // 负责pull　resquest
    ...
    for (;;)
    {
        for (int i = 0; i < remoteServerNum; i++)
        {
            ...
            std::string res = string(static_cast<const char *>(message.data()), message.size());
            remoteMsgs[i] = res;
            MOT_LOG_INFO((string("2.接收到消息:")+res).c_str());
        }
        while (MOTAdaptor::GetPreCounter() != 0);
        int index = 0;
        for (int i = 0; i < remoteServerNum; i++)
        {
            ...
            if (remoteMsgs[i] == " ")
                continue;
            MOT_LOG_INFO("2.execution stage");
            mergereq.ParseFromString(remoteMsgs[i]);
            ...
            for (int i = 0; i < mergereq.txn_size(); i++)
            {
                ...
                for (int j = 0; j < Txn.row_size(); j++)
                {
                    ...
                    MOT::Key *key = new MOT::Key(KeyLength);
                    key->CpKey((uint8_t *)Txn.row(j).key().c_str(), KeyLength);
                    MOT_LOG_INFO(("2.recv_key:" + key->GetKeyStr()).c_str());
                    MOT_LOG_INFO(("2.recv_tableName" + Txn.row(j).tablename()).c_str());
                    MOT::Row *localRow = nullptr;
                    uint64_t csn = Txn.committs();
                    table->FindRow(key, localRow, 0);
                    remote_changeset.insert(std::pair<int, MOT::Row *>(index++, localRow));
        ...
        for (int i = 0; i < remoteServerNum; i++)
        {
            ...
            for (int i = 0; i < mergereq.txn_size(); i++)
            {
                merge::MergeRequestNew_Transaction Txn = mergereq.txn(i);
                int commitNum = Txn.row_size();
                for (int j = 0; j < Txn.row_size(); j++)
                {
                    MOT::Row *localRow = remote_changeset.find(index++)->second;
                ...
                if (commitNum == Txn.row_size())
                {
                    ...
                    for (int j = 0; j < Txn.row_size(); j++)
                    {
                        ...
                        if (localRow != nullptr)
                        {
                            ...
                            MOT_LOG_INFO("It is not null ptr!!");

                            int col_size = Txn.row(j).col_size();

                            MOT_LOG_INFO((string("col_sizeeeeeee:")+std::to_string(col_size)).c_str());
                            ...
                            if (col_size == 0)
                                ...
                                // localRow->CopyData((uint8_t *)Txn.row(j).data().c_str(), table->GetTupleSize());
                                {
                                    ...
                                    MOT_LOG_INFO((string("2.(WR)Modified col_size = ")+std::to_string(col_size)).c_str());
                                    for (int k=0; k < col_size; k++)
                                    {
                                        merge::MergeRequestNew_Transaction_Row_Column col = Txn.row(j).col(k);
                                        MOT_LOG_INFO("Before SetValueVariable_1....");
                                        //localRow->SetValueVariable(col.id(),col.value().c_str(),table->GetFieldSize(col.id()));
                                        
                                       localRow->SetValueVariable_1(col.id(),col.value().c_str(),col.value().length());
                                        
                                        
                                        MOT_LOG_INFO("Done SetValueVariable_1....");
                                        MOT_LOG_INFO((string("2.(WR)ColId = ")+std::to_string(col.id())+string(", col_size = ")+std::to_string(col.value().length())+
                                        string(",table_col_size=") + std::to_string(table->GetFieldSize(col.id())) 
                                        + string(",offset = ") + std::to_string(localRow->GetTable()->GetFieldOffset(col.id()))
                                        ).c_str());
                        ...
                        {
                            //插入新row
                            MOT_LOG_INFO("2.Here is what whould be inserted...");
                             int col_size = Txn.row(j).col_size();
                            MOT_LOG_INFO((string("2.(INS)Modified col_size = ")+std::to_string(col_size)).c_str());
                            for (int k=0; k < col_size; k++)
                            {
                                merge::MergeRequestNew_Transaction_Row_Column col = Txn.row(j).col(k);
                                //MOT_LOG_INFO((string("2.(INS)ColId = ")+std::to_string(col.id())+string(", Colvalue = ")+col.value()+string(", tableFieldSize = ")+std::to_string(col.value().length())).c_str());
                                 MOT_LOG_INFO((string("2.(INS)ColId = ")+std::to_string(col.id())+string(", col_size = ")+std::to_string(col.value().length())+
                                        string(",table_col_size=") + std::to_string(table->GetFieldSize(col.id()))
                                        ).c_str());
                            }
...
void MOTAdaptor::ProcessMergeThread_2(){
    MOT_LOG_INFO("2.进入ProcessMergeThread_1......")
    zmq::context_t context(1);
    int remoteServerNum = 1;
    //zmq_ctx_set(&context, ZMQ_IO_THREADS, 3);
    // 负责pull　resquest
    ...
    for(;;){
        for(int i = 0 ; i < remoteServerNum; i++){
            ...
            if (res.size() > 0)
                {MOT_LOG_INFO("2.对方此轮没有做事....");}
            else
                {MOT_LOG_INFO("2.======================对方此轮做事了==========================");}
        ...
        for(int i = 0 ; i < remoteServerNum; i++){
            ...
            merge::MergeRequestNew mergereq;
            /******************************************/
            if(remoteMsgs[i] == " ") continue;

            MOT_LOG_INFO("2.开始执行Merge操作....");
            MOT_LOG_INFO((string("2.接受到的message:")+remoteMsgs[i]).c_str());
            MOT_LOG_INFO("2.execution stage");
            mergereq.ParseFromString(remoteMsgs[i]);
            ...
            for(int i = 0; i < mergereq.txn_size(); i++){
                ...
                /******************************************/
                for(int j = 0; j < Txn.row_size(); j++){
                    MOT_LOG_INFO((string("2.打上cs阶段--row_size: ")+std::to_string(Txn.row_size())).c_str());
                    //MOT::Table * table = m_engine->GetTableManager()->GetTableByExternal(Txn.row(j).tableexid());
                    MOT::Table *table = m_engine->GetTableManager()->GetTable(Txn.row(j).tablename());
                    int KeyLength = Txn.row(j).key().length();
                    MOT::Key* key= new MOT::Key(KeyLength);
                    MOT_LOG_INFO((string("2.key_size = ")+std::to_string(KeyLength)).c_str());
                    MOT_LOG_INFO("2.ProcessMergeThread_1:在Cpkey之前");
                    key->CpKey((uint8_t*)Txn.row(j).key().c_str(),KeyLength);
                    MOT_LOG_INFO("2.ProcessMergeThread_1:在Cpkey之后");
                    MOT::Row * localRow=nullptr;
                    uint64_t csn = Txn.committs();
                    table->FindRow(key,localRow, 0);
                    remote_changeset.insert(std::pair<int, MOT::Row *>(index++, localRow));
        ...
        for(int i = 0 ; i < remoteServerNum; i++){
            ...
            for(int i = 0; i < mergereq.txn_size(); i++){
                ...
                int commitNum = Txn.row_size();
                for(int j = 0; j < Txn.row_size(); j++){
                    MOT_LOG_INFO((string("2.commit阶段--row_size: ")+std::to_string(Txn.row_size())).c_str());
                    MOT::Row * localRow = remote_changeset.find(index++)->second;
                    uint64_t csn = Txn.committs();
                    if(localRow!=nullptr){
                        MOT_LOG_INFO(std::to_string(csn).c_str());
                ...
                if(commitNum ==  Txn.row_size()){
                    ...
                    for(int j = 0; j < Txn.row_size(); j++){
                        ...
                        MOT_LOG_INFO((string("2.commit阶段--col_size: ")+std::to_string(Txn.row(j).col_size())).c_str());
                        MOT_LOG_INFO((string("=========接受信息对方的ip:")+mergereq.request_address()+string("==================")).c_str());
                        MOT_LOG_INFO((string("自己的Ip:")+get_local()).c_str());
                        //MOT::Table * table = m_engine->GetTableManager()->GetTableByExternal(Txn.row(j).tableexid());
                        MOT::Table *table = m_engine->GetTableManager()->GetTable(Txn.row(j).tablename());
                        MOT::Row * localRow = remote_changeset.find(index++)->second;
                        /**************************************/
                        ...
                        if(localRow!=nullptr){
                            if(Txn.row(j).data().length()==0)
                                ...
                                localRow->CopyData((uint8_t*)Txn.row(j).data().c_str(),table->GetTupleSize());  //将值更新（需要改写为列）
                        ...
                        if (localRow != nullptr)
                        {
                            ...
                            else
                            {
                                for (int k=0; k < col_size; k++)
                                {
                                    merge::MergeRequestNew_Transaction_Row_Column col = Txn.row(j).col(k);
                                    localRow->SetValueVariable(col.id(),col.value().c_str(),table->GetFieldSize(col.id()));
                                    MOT_LOG_INFO((string("2.ColId = ")+std::to_string(col.id())+string(", Colvalue = ")+col.value()+string(", tableFieldSize = ")+std::to_string(table->GetFieldSize(col.id()))).c_str());
...
//SET 大小
int MOTAdaptor::GetSetSize()
{
    return local_changeset.size();
}

//为local_changeset中插入一个txMan
void MOTAdaptor::InsertRowToSet(MOT::TxnManager *txMan)
{
    MOT_LOG_INFO("Start insert TxnManager to localchangeset");
    local_changeset.insert(txMan);
    MOT_LOG_INFO("插入后的set大小 = %lu", local_changeset.size());
}

//清空local_changeset
void MOTAdaptor::ClearChangeSet()
{
    local_changeset.clear();
}
void *MOTAdaptor::RequestMerge(void *params)
{
...
void *MOTAdaptor::ProcessMerge(void *params)
{
    //MOTAdaptor::ProcessMergeThread();
    MOTAdaptor::ProcessMergeThread_1();
}

int MOTAdaptor::startMergeThread()
{
    if (pthread_create(&pMergeProcess, NULL, ProcessMerge, NULL)) //创建线程,执行RequestMerge和ProcessMerge
    {
        return -1;
    }
    return 0;
    ...
    MOT::Column *col, Expr *expr, Datum datum, Oid type, uint8_t *data, size_t len, KEY_OPER oper, uint8_t fill)
{
    ...
    case BPCHAROID:
    {
        ...
        { // OA: LLVM passes nullptr for expr parameter
            Const *c = (Const *)expr;
...
{
    if (m_end == -1 && m_ixOpers[0] == READ_KEY_EXACT)
    { // READ_KEY_EXACT
        return true;
    }
