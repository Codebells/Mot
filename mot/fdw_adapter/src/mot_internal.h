...
{
    // local transaction execute phase
    EXECUTE,
    // local transaction commit phase
    COMMIT,
};
...
private:
    ...
    // static MergeStatus status;
    static uint64_t epoch;
    static std::atomic<uint64_t> exeCounter;
    static std::atomic<uint64_t> preCounter;
    static std::atomic<uint64_t> comCounter;
    static pthread_t pTimer;
    static bool timerStop;
    static bool localExeced;//  local exeucte 
    static bool remoteComitted;//  remote commit
    static bool remoteExeced;// remote execute
    static bool mergeSent; // merge sent
    static bool inMerge;
...
public:
    ...
    ///
    static void* TimerThread(void* params);

    static void callback();

    static int startTimerThread(){
        //创建定时器线程
        if(pthread_create(&pTimer,NULL,TimerThread,NULL))
        {      
            return -1;
        }      
        return 0;
    ...
    static void setTimerStop(bool value) {
        timerStop = value;
    }

    static bool isTimerStop() {return timerStop;}

    static void setLocalExeced(bool value) {
        localExeced = value;
    }
    static bool isLocalExeced() {return localExeced;}


    static bool isRemoteExeced() {return remoteExeced;}

    static void setRemoteExeced(bool value) {
        remoteExeced = value;
    }

    static bool isRemoteCommited() {return remoteComitted;}

    static void setRemoteCommited(bool value) {
        remoteComitted = value;
    }

    static bool isMergeSent() {return mergeSent;}

    static void setMergeSent(bool value) {
        mergeSent = value;
    }
    static bool isInMerge(){return inMerge;}

    static void setInMerge(bool value){
        inMerge = value;
    }

    static bool add(MOT::TxnManager *txnManager)
    {
        return txnBuffer.insert(txnManager).second;
    }
    ...
    static TxnBufferIter begin()
    {
        return txnBuffer.begin();
    }

    static TxnBufferIter end()
    {
        return txnBuffer.end();
    }

    // static void SetStatus(MergeStatus _status)
    // {
    //     status = _status;
    // }

    // static MergeStatus GetStatus() { return status; }

    // // static void IncEpoch() { epoch++; }


    static void IncEpoch() { epoch++; }

    static uint64_t GetEpoch() {return epoch;}

    static void IncExeCounter()
    {
        exeCounter += 1;
    }
    static void DecExeCounter()
    {
        exeCounter -= 1;
    }
     static uint64_t GetExeCounter() {
        return exeCounter.load();
    }

    static void IncPreCounter()
    {
        preCounter += 1;
    }
    static void DecPreCounter()
    {
        preCounter -= 1;
    }
     static uint64_t GetPreCounter() {
        return preCounter.load();
    }

    static void IncComCounter()
    {
        comCounter += 1;
    }
    static void DecComCounter()
    {
        comCounter -= 1;
    }
     static uint64_t GetComCounter() {
        return comCounter.load();
    }
    ...
    //保存更新的集合
    static unordered_set<MOT::TxnManager*> local_changeset;

    //保存remote的集合
    static unordered_map<int, MOT::Row *> remote_changeset;
    
    //保存插入的sentinel
    static std::unordered_map<void*, uint64_t> insertSet;

    //发送merge的 pid;
    static pthread_t pMerge;
    //处理merge的 pid 
    static pthread_t pMergeProcess;

    //请求时间戳
    static uint64_t RequestTimeStamp();

    //获取changeset的size
    static int GetSetSize();

    //为local_changeset中插入一个txMan
    static  void InsertRowToSet(MOT::TxnManager* txnMa);

    //清空local_changeset
    static  void ClearChangeSet();
  
    //真正执行发送merge的函数
    static void RequestMergeThread();

    static void RequestMergeThread_1();

    static void RequestMergeThread_2();
   
    //执行merge process的函数
    static void ProcessMergeThread();
    
    static void ProcessMergeThread_1();

    static void ProcessMergeThread_2();

    //发送merge请求的函数,线程绑定的
    static void * RequestMerge(void * params);

    //处理merge的函数，线程创建时绑定的全局函数
    static void * ProcessMerge(void * params);
    
    //启动merge
    static int startMergeThread();
